                        ...
                        Super_Comp)            #Is this being run on the super computer?
{
  ...
  for (i in 1:(dim(N_wSpace_postM_alt)[1]-6)){
    for (j in 1:dim(N_wSpace_postM_alt)[2]){ 
     mod_alt$N_bar[i,j,]<-(N_wSpace_postM_alt[i,j,]*(1-exp(-(M_vec[j]+F_space_alt[i,j,]))))/(M_vec[j]+F_space_alt[i,j,])  
    }
  }
  ...
  #############################################
  #Adding Survey index and Composition
  #############################################
