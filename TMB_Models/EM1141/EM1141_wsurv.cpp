...
//EM1141 (2D AR(1) M devs, with Dirichlet-multinomial)
...
//    for(j=fage;j<=lage;j++){
   for(i=fage;i<=8;i++){
    for(j=fage;j<=8;j++){
     ...
     Ra(i,j) =  pow(rho_Ma_logis,indicator_mat_a(i,j));       //covariance matrix of only rho
    }
   }
   ...
   for(i=0;i<=lyear-1;i++){
    for(j=0;j<=lyear-1;j++){
     ...
     Rt(i,j) =  pow(rho_Mt_logis,indicator_mat_yr(i,j));       //covariance matrix of only rho
    }
   }
...
//  Rtotal=kronecker(Ra,Rt);
  ...
  for(i=0;i<=9*years.size()-1;i++){
   for(j=0;j<=9*years.size()-1;j++){
    varcov(i,j)=exp(log_lM_sd)*Rtotal(i,j)*exp(log_lM_sd);
   }
  }
  
  //Recruitment deviations
   NPRAND -= sum(dnorm(log_recruit_devs,0,sd_rec,true)); //Recruitment deviations
   NPRAND += MVNORM(varcov)(log_M_devs); //M deviations
