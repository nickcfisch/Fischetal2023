...
//EM3111 (Xu sel devs and RW M, with Dirichlet-multinomial)
...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER(logsd_lM_RWdevs);         //sd of log q devs  
  PARAMETER_VECTOR(log_M_RWdevs);            //log_q_devs
  //---------------------------------------------------------------------------------------------
...
//  matrix<Type> Ra(ages.size(),ages.size());
  matrix<Type> Rt(years.size(),years.size());
//  matrix<Type> Rtotal(ages.size()*years.size(),ages.size()*years.size());
 
  matrix<Type> indicator_mat_a(9,9);                          //only doing ages 2-10
  matrix<Type> Ra(9,9);
  matrix<Type> Rtotal(9*years.size(),9*years.size());
  matrix<Type> varcov(9*years.size(),9*years.size());
  ...
  vector<Type> M_scalar_yr(years.size());
...
//////////////////////
 //First getting size at age for Lorenzen M
  ...
  M_scalar_yr(0)=exp(log_M_scalar);
  for(i=1;i<=lyear-1;i++){
   M_scalar_yr(i)=M_scalar_yr(i-1)*exp(log_M_RWdevs(i-1));
  }
...
//    for(j=fage;j<=lage;j++){
   for(i=fage;i<=8;i++){
    for(j=fage;j<=8;j++){
     ...
     Ra(i,j) =  pow(rho_sela_logis,indicator_mat_a(i,j));       //covariance matrix of only rho
    }
   }
   ...
   for(i=0;i<=lyear-1;i++){
    for(j=0;j<=lyear-1;j++){
     ...
     Rt(i,j) =  pow(rho_selt_logis,indicator_mat_yr(i,j));       //covariance matrix of only rho
    }
   }

//  Rtotal=kronecker(Ra,Rt);
  Rtotal=kronecker(Rt,Ra);

  for(i=0;i<=9*years.size()-1;i++){
   for(j=0;j<=9*years.size()-1;j++){
    varcov(i,j)=exp(log_lsel_sd)*Rtotal(i,j)*exp(log_lsel_sd);
   }
  }
  
  //Recruitment deviations
   NPRAND -= sum(dnorm(log_recruit_devs,0,sd_rec,true)); //Recruitment deviations
   NPRAND += MVNORM(varcov)(log_sel_devs); //Sel deviations
   NPRAND -= sum(dnorm(log_M_RWdevs,0,exp(logsd_lM_RWdevs),true)); //Catchability RW deviations
