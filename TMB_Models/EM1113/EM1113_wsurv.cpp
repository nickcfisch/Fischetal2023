...
//EM1113 (Constant everything, with Multinomial)
...
//   N(fyear-1,j)=R0*exp(log_recruit_devs(j)-0.5*pow(sd_rec,2))*lxo(j);           //set abundance in the fist year f
  }
...
//   N(i,fage)=((4.*steepness*R0*spbiomass(i))/(SSB0_FLA*(1.-steepness)+spbiomass(i)*(5.*steepness-1.)))*exp(log_rec_devs(i+lage)-0.5*pow(sd_rec,2));
 }
...
//Fishery CPUE
  for(i=0;i<=lyear-1;i++){
   L3 += log(cv_fishery_CPUE*pred_fishery_cpue(i))+0.5*pow((obs_fishery_cpue(i)-pred_fishery_cpue(i))/(cv_fishery_CPUE*pred_fishery_cpue(i)),2); //Likelihood for Fishery CPUE
  }
...
//Fisheries-independent likelihoods
  for(i=(lyear/2);i<=lyear-1;i++){
   if(SS_FIM(i)>0){
    for(j=0;j<=lage;j++){
     L4 += -1*(SS_FIM(i)*(obs_FIM_comp(i,j)*log(pred_FIM_comp(i,j))));     //Likelihood for age composition of survey catch
    }
   }
   L5 += log(cv_FIM_CPUE*pred_totcatch_FIM(i)+0.0001)+0.5*pow((obs_FIM_CPUE(i)-pred_totcatch_FIM(i))/(cv_FIM_CPUE*pred_totcatch_FIM(i)),2); //Likelihood for FIM CPUE
  }
