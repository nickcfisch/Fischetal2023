...
//EM1112 (Constant everything, with Logistic-normal), no relative sample size given to comp weighting
...
// Fishery
  for(i=0;i<=lyear-1;i++){
   if(SS_fishery(i)>0){
    for(j=fage;j<=lage;j++){
      for(k=fage;k<=lage;k++){
       covmat(j,k) = covmat_rho(j,k)*pow(sig_LN,Type(2));   //bringing in sigma to Cov matrix
	  }
	}
    ...
    L2_comp(i) = Type(0.5)*(lage-fage)*log(Type(2)*pi)+log(obs_fishery_comp.array()).row(i).sum()+Type(0.5)*atomic::logdet(Vmat)+(lage-fage)*log(1)+Type(0.5)*(LN_seg1.sum()/pow(1,Type(2)));
   }
  }
...
//Survey 
  for(i=(lyear/2);i<=lyear-1;i++){
   if(SS_FIM(i)>0){
    for(j=fage;j<=lage;j++){
     for(k=fage;k<=lage;k++){
      covmat_surv(j,k) = covmat_surv_rho(j,k)*pow(sig_LN_surv,Type(2));     //adding sig^2 into covariance matrix
	 }
	}
   ...
   Vinv_surv = atomic::matinv(Vmat_surv);                            //inverse of Vmat
    ...
    L4_comp(i) = Type(0.5)*(lage-fage)*log(Type(2)*pi)+log(obs_FIM_comp.array()).row(i).sum()+Type(0.5)*atomic::logdet(Vmat_surv)+(lage-fage)*log(1)+Type(0.5)*(LN_surv_seg1.sum()/pow(1,Type(2)));
   }
  }
...
//Fisheries-independent likelihoods
  ...
  //Recruitment deviations
   NPRAND -= sum(dnorm(log_recruit_devs,Type(0),sd_rec,true)); //Recruitment deviations
