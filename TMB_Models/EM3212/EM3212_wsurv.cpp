...
//EM2112 (IID sel devs, constant q, with Logistic-normal)
...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER(logsd_lq_devs);         //sd of log q devs  
  PARAMETER_VECTOR(log_q_devs);            //log_q_devs
...
////////////////
  ...
  for(i=0;i<=lyear-1;i++){
   q_time(i)=exp(log_q)*exp(log_q_devs(i));
...
//  Rtotal=kronecker(Ra,Rt);
  ...
  //Recruitment deviations
   ...
   NPRAND -= sum(dnorm(log_q_devs,0,exp(logsd_lq_devs),true)); //Catchability iid deviations
