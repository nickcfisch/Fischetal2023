...
//EM1114 (Constant everything, with Logistic-normal, but with no correlation [only sigma in diagonal])
  ...
  for(i=(lyear/2);i<=lyear-1;i++){
   ...
   for(i=fage;i<=lage;i++){
    for(j=fage;j<=lage;j++){
     ...
     covmat_rho(i,j) =  pow(0,indicator_mat(i,j));       //covariance matrix of only rho
     covmat_surv_rho(i,j) =  pow(0,indicator_mat(i,j));       //covariance matrix of only rho
    }
   }
