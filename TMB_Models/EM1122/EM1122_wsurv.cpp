...
//EM1122 (M scalar IID devs, with Logistic-normal)
...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER(logsd_lM_devs);         //sd of log q devs  
  PARAMETER_VECTOR(log_M_devs);            //log_q_devs
  //---------------------------------------------------------------------------------------------
...
//////////////////////
 ...
 //Filling in natural mortality matrix
  for(i=0;i<=years.size()-1;i++){
   ...
   for(j=2;j<=lage;j++){
    M(i,j)=M_scalar*exp(log_M_devs(i))*Lt(7)/Lt(j); 	
   }
  }
...
//Fisheries-independent likelihoods
  ...
  //Recruitment deviations
   ...
   NPRAND -= sum(dnorm(log_M_devs,Type(0),exp(logsd_lM_devs),true)); //Catchability iid deviations
