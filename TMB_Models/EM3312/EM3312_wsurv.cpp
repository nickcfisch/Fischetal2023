...
//EM3312 (Xu sel devs, rw q, with Logistic-normal)
...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER(logsd_lq_RWdevs);         //sd of log q devs  
  PARAMETER_VECTOR(log_q_RWdevs);            //log_q_devs
...
//////////////////////
 ...
 //Filling in natural mortality matrix
  for(i=0;i<=years.size()-1;i++){
   M(i,0)=Type(2.0);   //Filling in age 0 M 
   M(i,1)=Type(1.2);   //Filling in age 1 M, rest is based on Lorenzen
   for(j=2;j<=lage;j++){
...
////////////////
  ...
  q_time(0)=exp(log_q);
  for(i=1;i<=lyear-1;i++){
   q_time(i)=q_time(i-1)*exp(log_q_RWdevs(i-1));
  }
...
//  Rtotal=kronecker(Ra,Rt);
  ...
  //Recruitment deviations
   ...
   NPRAND -= sum(dnorm(log_q_RWdevs,0,exp(logsd_lq_RWdevs),true)); //Catchability RW deviations
