...
//EM1131 (RW M scalar, with Dirichlet-multinomial)
...
//Prior on DM thetas, hyperbolic
  Type pi=3.141593;
  Type mu=Type(0);
  Type alpha=1.123945;
  Type delta=2.211630;
  Type beta=Type(0);
  Type lamda=sqrt(pow(alpha,2)-pow(beta,2));
//Hyperbolic density
  Type L6 = -1*log( lamda/(2*alpha*delta*besselK(delta*lamda,Type(1))) * exp(-alpha*sqrt(pow(delta,2)+pow(ltheta-mu,2))+beta*(ltheta-mu)) );
  Type L7 = -1*log( lamda/(2*alpha*delta*besselK(delta*lamda,Type(1))) * exp(-alpha*sqrt(pow(delta,2)+pow(ltheta_FIM-mu,2))+beta*(ltheta_FIM-mu)) );
